...
class SubscribeTopicActivity < BasicActivity
  ...
  # Create an SQS queue and return the queue obj 
  ...
  def do_activity(task)
    activity_data = {
      ...
      :sqs => {:queue_url => nil, :queue_name => nil},
    }

    # Create an SQS client. This will subscribe to the SNS topic.
    sqs_client = AWS::SQS::Client.new(
      :config => AWS.config.with(:region => $SMS_REGION))
    q = create_a_queue(sqs_client)
    
    if task.input != nil
      input = YAML.load(task.input)
      activity_data[:email][:endpoint] = input[:email]
      activity_data[:sms][:endpoint] = input[:sms]
      activity_data[:sqs][:queue_url] = q[:queue_url]
      activity_data[:sqs][:queue_name] = q[:queue_url].split('/').last
    else
    ...
    queue = AWS::SQS.new.queues[q[:queue_url]]
    topic = AWS::SNS.new.topics[activity_data[:topic_arn]]
